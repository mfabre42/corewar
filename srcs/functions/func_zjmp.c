...
/*   Updated: 2017/05/16 18:01:14 by acoupleu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	jump_if_zero(t_map *map, t_process *proc)
{
	...
	else
	{
		...
		if (proc->carry == 1)
		{
			...
			printf("Maintenant, tu jump au numero: %d, pc: %d, start: %d au cycle %d\n", proc->pc + proc->start, proc->pc, proc->start, map->cycle);
		}
		else
			proc->pc += 3;
	}
