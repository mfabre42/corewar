...
void	direct_store(t_map *map, t_process *proc)
{
	...
	else
	{
		...
		if (ocp.param2 == 'R')
		{
			if (!is_reg(hex_to_int(0x00, 0x00, 0x00, ARENA[(PPOS + 3)
			% MEM_SIZE]), proc, 4))
				return ;
			proc->reg[(int)ARENA[(PPOS + 3) % MEM_SIZE] - 1] =  proc->reg[reg_nbr - 1];
			proc->pc = proc->pc + 4;
		...
		else
		{
			result = PPOS + (short)hex_to_int(0x00, 0x00,
				ARENA[(PPOS + 3) % MEM_SIZE], ARENA[(PPOS + 4) % MEM_SIZE]) % IDX_MOD;;
