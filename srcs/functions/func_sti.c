...
/*   Updated: 2017/05/24 01:15:26 by acoupleu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		indirect_store2(t_map *map, t_process *proc, t_bin *bin, int *result)
{
	...
	{
		if (!is_reg((int)map->arena[(POS + 3) % MEM_SIZE], proc, 6))
			return 0;
	...
	{
		if (!is_reg((int)map->arena[(POS + 3) % MEM_SIZE], proc, 5)
...
void	indirect_store(t_map *map, t_process *proc)
{
	...
	else
	{
		...
		POCP = ocp_master((int)map->arena[(PPOS + 1) % MEM_SIZE]);
		if (!is_reg((int)map->arena[(PPOS + 2) % MEM_SIZE], proc, 5))
			return ;
		...
		{
			result = (short)hex_to_int(0x00, 0x00, map->arena[(PPOS + 3) % MEM_SIZE], map->arena[(PPOS + 4) % MEM_SIZE]);
			result += (short)hex_to_int(0x00, 0x00, map->arena[(PPOS + 5) % MEM_SIZE], map->arena[(PPOS + 6) % MEM_SIZE]);
			proc->pc = proc->pc + 7;
		}
		else if ((POCP.param2 == 'D' || POCP.param2 == 'I') && POCP.param3 == 'R')
		{
			if (!is_reg((int)map->arena[(PPOS + 5) % MEM_SIZE], proc, 6))
				return ;
			result = (short)hex_to_int(0x00, 0x00, map->arena[(PPOS + 3) % MEM_SIZE], map->arena[(PPOS + 4) % MEM_SIZE]);
			result += proc->reg[(int)map->arena[(PPOS + 5) % MEM_SIZE] - 1];
			proc->pc = proc->pc + 6;
		}
		else
		{
