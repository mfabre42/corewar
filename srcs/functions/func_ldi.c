...
void	indirect_load(t_map *map, t_process *proc)
{
	...
	else
	{
		...
		if (ocp.param1 == 'R' && ocp.param2 == 'R')
		{
			...
			proc->reg[(int)ARENA[(pos + 4) % MEM_SIZE] - 1] = read_in_arena(map, (pos + (result % IDX_MOD)) % MEM_SIZE);
			proc->pc = proc->pc + 5;
		}
		else if (ocp.param1 == 'R' && ocp.param2 == 'D')
		{
			...
			proc->reg[(int)ARENA[(pos + 5) % MEM_SIZE] - 1] = read_in_arena(map, (pos + (result % IDX_MOD)) % MEM_SIZE);
			proc->pc = proc->pc + 6;
		}
		else if ((ocp.param1 == 'D' || ocp.param1 == 'I') && ocp.param2 == 'R')
		{
			...
			|| !is_reg((int)ARENA[(pos + 5) % MEM_SIZE], proc, 6))
				return ;
			result = (short)hex_to_int(0x00, 0x00, ARENA[(pos + 2) % MEM_SIZE], ARENA[(pos + 3) % MEM_SIZE]);
		...
		else if ((ocp.param1 == 'D' || ocp.param1 == 'I') && ocp.param2 == 'D')
		{
			...
			proc->reg[(int)ARENA[(pos + 6) % MEM_SIZE] - 1] = read_in_arena(map, (pos + (result % IDX_MOD)) % MEM_SIZE);
			proc->pc = proc->pc + 7;
		}
	}
}
