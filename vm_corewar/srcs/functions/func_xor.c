...
/*   Updated: 2017/05/24 01:14:17 by acoupleu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "corewar.h"

static int	func_xor3(t_map *map, t_process *proc, t_bin *bin, int *param1)
{
	if (OCP.param2 == 'R')
	{
...
static int	func_xor2(t_map *map, t_process *proc, t_bin *bin, int *param1)
{
	if (OCP.param1 == 'R')
	{
...
void		func_xor(t_map *map, t_process *proc)
{
	...
	else
	{
		...
		if (func_xor2(map, proc, &bin, &param1) == 0)
			return ;
		if (func_xor3(map, proc, &bin, &param1) == 0)
			return ;
