...
/*   Updated: 2017/06/01 18:46:58 by acoupleu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "corewar.h"

static int	func_or3(t_map *map, t_process *proc, t_bin *bin, int *param1)
{
	if (OCP.param2 == 'R')
	{
...
static int	func_or2(t_map *map, t_process *proc, t_bin *bin, int *param1)
{
	if (OCP.param1 == 'R')
	{
...
void		func_or(t_map *map, t_process *proc)
{
	...
	else
	{
		...
		if (func_or2(map, proc, &bin, &param1) == 0)
			return ;
		if (func_or3(map, proc, &bin, &param1) == 0)
			return ;
