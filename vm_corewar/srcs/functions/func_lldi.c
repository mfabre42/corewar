...
/*   Updated: 2017/05/24 02:13:46 by acoupleu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "corewar.h"

static int	lindirect_load3(t_map *map, t_process *proc, t_bin *bin, int *resul)
{
	if ((OCP.param1 == 'D' || OCP.param1 == 'I') && OCP.param2 == 'R')
	{
		...
		|| !is_reg((int)ARENA[(POS + 5) % MEM_SIZE], proc, 6))
			return (0);
		*resul = (short)hex_to_int(0x00, 0x00, ARENA[(POS + 2) % MEM_SIZE],
		ARENA[(POS + 3) % MEM_SIZE]);
		*resul += proc->reg[(int)ARENA[(POS + 4) % MEM_SIZE] - 1];
	...
	else if ((OCP.param1 == 'D' || OCP.param1 == 'I') && OCP.param2 == 'D')
	{
		...
		*resul = (short)hex_to_int(0x00, 0x00, ARENA[(POS + 2) % MEM_SIZE],
		ARENA[(POS + 3) % MEM_SIZE]);
		*resul += (short)hex_to_int(0x00, 0x00, ARENA[(POS + 4) % MEM_SIZE],
